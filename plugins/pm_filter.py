...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("☜ɮǟƈӄ", callback_data=f"next_{req}_{key}_{off_set}"),
             InlineKeyboardButton(f"♛℘ąɠɛ♛ {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}",
                                  callback_data="pages")]
        )
    elif off_set is None:
        btn.append(
            [InlineKeyboardButton(f"♛{math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}♛", callback_data="pages"),
             InlineKeyboardButton("ռɛӼȶ☞", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
                InlineKeyboardButton("☜ɮǟƈӄ", callback_data=f"next_{req}_{key}_{off_set}"),
...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('<b>Plz Check Spelling Or Request At With @admin Command</b> \n')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🔍 Search', url='https://t.me/UrvashiTheaters'),
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"♛1/{math.ceil(int(total_results) / 10)}♛", callback_data="pages"),
             InlineKeyboardButton(text="ռɛӼȶ☞", callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
